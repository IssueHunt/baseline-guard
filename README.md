...
nonexistent-crate`). We tried to audit and couldn't, so default is
   fail-closed. Allow specific names via `baseline-guard.json#whitelist`
...
4. **Non-registry-spec check** (blocks by default): the user typed `file:`,
   ...
   `baseline-guard.json#whitelist` or use `["*"]` for blanket permission.
...
add them to `baseline-guard.json` under `whitelist` (see below). Too-fresh
packages can also be whitelisted by `name` or `name@version`. Malicious-package
matches never bypass the whitelist — they are a hard block.
...
```json
{
  ...
  "whitelist": [
    "file:./shared",
    "github:our-org/private-pkg",
    "git+https://internal-git.corp/team/*.git",
    "@company/*",
    "internal-pkg-*",
    "lodash@4.17.21"
  ]
...
A single `whitelist` array gates **all** non-malicious block categories
(non-registry specs, unresolvable names, unverifiable publish dates,
too-fresh packages). Each entry is matched against any of:

- the bare package **name** (e.g. `lodash`, `@company/sdk`)
- the typed/raw **spec** (e.g. `github:our-org/private-pkg`, `file:./shared`)
- `name@version` (e.g. `lodash@4.17.21`)

Any match allows. `*` glob is supported (`"@company/*"`, `"github:our-org/*"`).
`["*"]` allows everything except malicious-package matches (those are a hard
block and never bypassable).

The file is loaded from the cwd where you run `baseline guard ...`; override
with `--config <path>`. There's no CLI escape hatch — every grant must be a
committed config change for audit-trail reasons.
